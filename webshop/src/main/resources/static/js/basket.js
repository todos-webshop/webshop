...
function showBasket(jsonData) {
  ...
  for (var i = 0; i < jsonData.basketItems.length; i++) {
    var divRow = document.createElement("div");

    var codeDiv = document.createElement("div");
    codeDiv.innerHTML = jsonData.basketItems[i].product.code;
    codeDiv.setAttribute('class', 'div_class');
    divRow.appendChild(codeDiv);

    var nameDiv = document.createElement("div");
    nameDiv.innerHTML = jsonData.basketItems[i].product.name;
    nameDiv.setAttribute('class', 'div_class');
    divRow.appendChild(nameDiv);

    var addressDiv = document.createElement("div");
    addressDiv.innerHTML = jsonData.basketItems[i].product.address;
    addressDiv.setAttribute('class', 'div_class');
    divRow.appendChild(addressDiv);

    var manufacturerDiv = document.createElement("div");
    manufacturerDiv.innerHTML = jsonData.basketItems[i].product.manufacturer;
    manufacturerDiv.setAttribute('class', 'div_class');
    divRow.appendChild(manufacturerDiv);

    var priceDiv = document.createElement("div");
    priceDiv.innerHTML = jsonData.basketItems[i].product.price + " Ft";
    priceDiv.setAttribute('class', 'div_class');
    divRow.appendChild(priceDiv);

    var imgDiv = document.createElement("div");
    imgDiv.innerHTML = "<img alt=" + jsonData.basketItems[i].product.address + " src=img\\products\\" + jsonData.basketItems[i].product.address + ".png>";
